...
Usage
-----
    ./ivy-python scripts/vet_x_handles.py --candidates            # the shortlist below
    ./ivy-python scripts/vet_x_handles.py handleA handleB ...     # your own list
    ./ivy-python scripts/vet_x_handles.py --candidates --json out.json
...
def report(results):
    """Print a per-handle verdict. Returns the handles worth adding."""
    keep = []
    print("\n" + "=" * 64)
    print("VERDICT")
    print("=" * 64)
    for handle, picks in results.items():
        if not picks:
            print(f"  ✗ @{handle:<20} no bettable picks returned — do not add")
            continue
        keep.append(handle)
...
def main(argv=None):
    ...
    handles = args.handles or (CANDIDATES if args.candidates else [])
    if not handles:
        ap.error("give handles, or pass --candidates")
