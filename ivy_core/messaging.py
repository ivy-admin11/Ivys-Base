...
def send_imessage_attachment(
    ...
    *,
    ...
    if locked and "paste" in order:
        logger.info("Screen is locked — skipping keystroke-based paste delivery for %s", phone_number)
        order.remove("paste")
    if not order:
        order = ["scripting"]
    ...
    for method in order:
        ...
        if last_result != "SUCCESS":
            ...
            logger.warning(
                "send_imessage_attachment [%s] attempt %d failed for %s: %s",
                method, attempts, phone_number, last_result,
            )
            continue
        ...
        logger.info(
            "send_imessage_attachment [%s] attempt %d for %s → %s (%s)",
            method, attempts, phone_number, outcome, details.get("source"),
        )
        ...
        logger.warning("send_imessage_attachment [%s] did not deliver to %s: %s", method, phone_number, last_error)
