...
def _chat_db_path() -> str:
    """Resolved per call, not at import, so tests can point this at a scratch
    path and never touch the real Messages database."""
    return os.path.expanduser(os.environ.get("CHAT_DB_PATH", "~/Library/Messages/chat.db"))


def _gateway_base_url() -> str:
    """Resolved per call for the same reason: the test suite points this at a
    closed port so a unit test can never query the live gateway."""
    return os.environ.get("IVY_GATEWAY_URL", "http://127.0.0.1:8000").rstrip("/")
...
def _lookup(
    *, since_ts: float, filename: Optional[str], handle: Optional[str]
    ...
    try:
        ...
        resp = requests.get(
            f"{_gateway_base_url()}/imessage/attachments",
